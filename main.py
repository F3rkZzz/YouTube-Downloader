...
class gui(QtWidgets.QMainWindow):
    def __init__(self,parent=None):
        ...
        self.ui.Button_2.clicked.connect(self.get_folder)
        self.ui.Button_1.clicked.connect(self.start)
        self.mythread=downloader()
        self.mythread.mysignal.connect(self.handler)
    def start(self):
        if len(self.ui.linkspace.text())>5:
            if self.download_folder != None:
                link=self.ui.linkspace.text()
    ...
    def handler(self, value):
        if value=="finish":
            ...
            self.ui.infoblock.appendPlainText(value)
    def locker(self,lock_value):
        base=[self.ui.Button_2, self.ui.Button_1]
        for item in base:
